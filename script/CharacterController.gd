...
# 路点跟随（多段移动，供 WaypointGraph/RendezvousManager 等宏观路径规划使用）
var waypoint_queue: Array = []  # 待经过的坐标队列，move_to 每次只负责走到队首
var speed_scale: float = 1.0  # 速度缩放系数，由 RendezvousManager 用于同步两人到达时间
...
func _ready():
	...
	# 如果角色是在世界暂停期间创建的，立即应用灰化效果
	var pause_manager = get_node_or_null("/root/PauseManager")
	if pause_manager:
		pause_manager.apply_pause_visual(self)
...
# 沿一系列坐标依次移动（宏观路点路径），每一段仍然通过 move_to() 使用
# navmesh + 避障来走细节位移，只是把最终目标依次替换为路径上的下一个路点。
# 常用于 WaypointGraph.get_path_positions() 或 RendezvousManager 规划出的路径。
func move_along_waypoints(points: Array) -> void:
	waypoint_queue.clear()
	if points.is_empty():
		return
	waypoint_queue = points.duplicate()
	var first_point = waypoint_queue.pop_front()
	move_to(first_point)

# 设置移动速度缩放系数（用于 RendezvousManager 让较快的一方降速，
# 从而与另一位角色同步到达相遇点）。
	...
	if input_direction != Vector2.ZERO:
		...
		if path_index < navigation_path.size():
			...
			if path_index == navigation_path.size() - 1 and distance_to_target < 50:
				adjusted_speed = lerp(adjusted_speed * 0.4, adjusted_speed, distance_to_target / 50)
			...
			if not waypoint_queue.is_empty():
				# 路点队列未走完，继续前往下一个路点
				var next_waypoint = waypoint_queue.pop_front()
				move_to(next_waypoint)
			else:
				speed_scale = 1.0
				waypoint_path_finished.emit()
				# 检查是否需要自动坐下
